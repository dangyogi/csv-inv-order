...
def run():
    ...
    for item, counts in groupby(sorted(get_counts()), key=itemgetter(0)):
        ...
        for item, start_date, start_count, end_date, end_count in counts:
            ...
            print(f"  {item=}, start={start_date:'%b %d, %y'}, end={end_date:'%b %d, %y'}, "
                  f"served={total_served}, months={total_months}, {consumed=:.02f}", end='')
            if total_served:
                print(f", per_serving={consumed/total_served:.02f}")
            else:
                print()
